...
struct MATERIAL
{
	...
	D3DXCOLOR	Diffuse;	//拡散反射
	D3DXCOLOR	Specular;	//鏡面の反射
	D3DXCOLOR	Emission;	//自己発光
	float		Shininess;
	int			noTexSampling;	//テクスチャの有無
	float		Dummy[2];	//16byte境界用
};

// ライト構造体
struct LIGHT {
	D3DXVECTOR4 Direction;	// ライトの方向	//平行光源
	D3DXVECTOR4 Position;	// ライトの位置	//点光源
	D3DXCOLOR	Diffuse;	// 拡散光の色	//光の色
	D3DXCOLOR   Ambient;	// 環境光の色	//光のないところの色
	float		Attenuation;// 減衰率		//点光源
	int			Type;		// ライト種別
